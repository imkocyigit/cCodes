...
/*
	...
	for(i=20;i<70;i++) {
		cairo_move_to(cr,i*5,(readADC[i-20]/10)+12);
		cairo_line_to(cr,(i+1)*5,(readADC[i-19]/10)+12);
	}
	cairo_set_line_width(cr,2);
	cairo_stroke(cr);
	...
	for(i=20;i<69;i++) {
		cairo_move_to(cr,i*5,(readFDEV[i-20]*10)+130);
		cairo_line_to(cr,(i+1)*5,(readFDEV[i-19]*10)+130);
	}
	cairo_set_line_width(cr,2);
	cairo_stroke(cr);
	...
	for(i=20;i<67;i++) {
		cairo_move_to(cr,i*5,(readSDEV[i-20]*10)+260);
		cairo_line_to(cr,(i+1)*5,(readSDEV[i-19]*10)+260);
	}
...
int main(int argc, char **argv) {
	...
	//gtk_widget_set_size_request(darea,50,50);
