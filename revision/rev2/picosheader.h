...
struct rets {
        ...
        int freqint,mfi1,mff1,mfi2,mff2,vpp,vmax,vmin;
        int *adc;
        int *fdev;
        int *sdev;
        char *message;
...
int * portreadN(void) {
	...
	printf("\nreading  took %f seconds to execute \n",time_taken);
...
struct rets* compose(int *p) {
	...
	for(i=0;i<50;i++) {tempadc[i]=adc[i];};	
	
	bubbleSort(tempadc,50);

	printf("\n BubbleSort : ");

	for(i=0;i<50;i++) { printf("%d ",tempadc[i]);};


	vmax=tempadc[49]/204;
	vmin=tempadc[0]/204;

	retvals->vpp=vmax-vmin;
	retvals->vmax=vmax;
	retvals->vmin=vmin;
	...
	printf("\ncomposing took %f seconds to execute \n",time_taken1);
